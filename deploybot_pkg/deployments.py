...
from .startup import deterministic_port, materialize_startup_points
...
def _run_fake_deploy(action: str, device, username: str, password: str, extra: list[str]) -> int | str:
    ...
    if action in {"deploy", "start", "start-custom", "stop", "start-tunnel", "stop-tunnel"} and completed.stdout:
        print(completed.stdout, end="")
    if completed.stderr:
        print(completed.stderr, end="")
    if action in {"deploy", "start", "start-custom", "stop", "start-tunnel", "stop-tunnel"}:
...
def _remote_post_copy_script(app_user: str, package_name: str, manifest: dict) -> str:
    ...
    install_steps.append(
        f"printf '%s\\n' \"$SUDO_PASS\" | sudo -S -u {shell_quote(app_user)} sh -lc 'cd {shell_quote(package_dir)} && if [ -f package.json ] && command -v npm >/dev/null 2>&1; then npm install --omit=dev; fi'"
    )
    install_steps.append(
...
def _load_remote_manifest(device, username: str, password: str, deployment: RemoteDeployment) -> tuple[int, dict | None]:
    fake_result = _run_fake_deploy("startup-points", device, username, password, [deployment.package_name])
    if fake_result != -1:
        ...
        try:
            ...
            return 1, None
        return 0, dict(payload.get("manifest", {}))

    completed = run_ssh_with_password(
        device,
        username,
        password,
        _load_remote_manifest_command(deployment.package_path),
        capture_output=True,
    )
    if completed.returncode != 0:
    ...
    try:
        return 0, json.loads(completed.stdout or "{}")
    except json.JSONDecodeError:
        print(f"Could not parse deploybot-manifest.json for {deployment.package_name}.")
        return 1, None


def _default_startup_points_for_deployment(deployment: RemoteDeployment) -> list[dict]:
    if deployment.runtime == "static-files":
        return [
            {
                "name": "web",
                "role": "primary",
                "command": f"python3 -m http.server {deterministic_port(deployment.package_name)} --bind 0.0.0.0",
                "path": "",
                "port": deterministic_port(deployment.package_name),
                "source": "default static runtime",
            }
        ]
    return []


def _startup_points_for_deployment(
    device,
    username: str,
    password: str,
    ...
    if fake_result != -1:
        ...
        try:
            ...
            print(f"Could not parse startup points for {deployment.package_name}.")
            return 1, None
        return 0, list(payload.get("startup_points") or [])

    code, manifest = _load_remote_manifest(device, username, password, deployment)
    if code != 0:
        return code, None
    if manifest is None:
        return 0, _default_startup_points_for_deployment(deployment)
    startup_points = materialize_startup_points(manifest, deployment.package_name)
    if not startup_points:
        startup_points = _default_startup_points_for_deployment(deployment)
    return 0, startup_points
...
def _start_command_for_deployment(deployment: RemoteDeployment, startup_points: list[dict]) -> tuple[str, int]:
    if not startup_points:
    ...
    primary_port = int(primary.get("port") or 0)
    ...
    command_lines = [
        ...
        f"    printf 'App already running on port %s\\n' {shell_quote(str(primary_port))}",
        "    exit 0",
        "  fi",
        "fi",
    ]

    pid_arg_indexes: list[int] = []
    python_args = ['"$RUNTIME_FILE"', '"$PACKAGE_DIR"', '"$APP_USER"']
    next_arg_index = 4
    if primary_port:
        command_lines.append(f"PRIMARY_PORT={primary_port}")
        python_args.append('"$PRIMARY_PORT"')
        primary_port_index = next_arg_index
        next_arg_index += 1
    else:
        primary_port_index = 0

    process_exprs: list[str] = []
    for index, point in enumerate(startup_points, start=1):
        safe_name = re.sub(r"[^a-zA-Z0-9_-]+", "-", str(point.get("name", f"process-{index}"))).strip("-") or f"process-{index}"
        command_text = str(point.get("command", "")).strip()
        pid_file = f"/tmp/{deployment.package_name}-{safe_name}.pid"
        log_file = f"/tmp/{deployment.package_name}-{safe_name}.log"
        command_lines.append(
            "printf '%s\\n' \"$SUDO_PASS\" | sudo -S -u \"$APP_USER\" sh -lc "
            + shell_quote(f"cd {package_dir} && nohup {command_text} >{log_file} 2>&1 & echo $! >{pid_file}")
        )
        command_lines.append(f"PID_{index}=$(printf '%s\\n' \"$SUDO_PASS\" | sudo -S cat {shell_quote(pid_file)})")
        command_lines.append(f"printf '%s\\n' \"$SUDO_PASS\" | sudo -S rm -f {shell_quote(pid_file)}")
        python_args.append(f'"$PID_{index}"')
        pid_arg_indexes.append(next_arg_index)
        next_arg_index += 1

        port_expr = "None"
        if point.get("port") is not None:
            if point is primary and primary_port_index:
                port_expr = f"int(sys.argv[{primary_port_index}])"
            else:
                port_expr = str(int(point["port"]))
        process_exprs.append(
            "{"
            f"'name': {point.get('name', '')!r}, "
            f"'role': {point.get('role', 'companion')!r}, "
            f"'command': {command_text!r}, "
            f"'port': {port_expr}, "
            f"'pid': int(sys.argv[{pid_arg_indexes[-1]}])"
            "}"
        )

    primary_pid_index = pid_arg_indexes[0]
    primary_port_expr = f"int(sys.argv[{primary_port_index}])" if primary_port_index else "0"
    command_lines.append(
        "printf '%s\\n' \"$SUDO_PASS\" | sudo -S -u \"$APP_USER\" python3 - <<'PY' "
        + " ".join(python_args)
    )
    command_lines.extend(
        [
            ...
            f"  'port': {primary_port_expr},",
            f"  'pid': int(sys.argv[{primary_pid_index}]),",
            f"  'processes': [{', '.join(process_exprs)}],",
            "}",
    ...
    if primary_port:
        command_lines.append("printf 'Started app as %s on port %s\\n' \"$APP_USER\" \"$PRIMARY_PORT\"")
    else:
        command_lines.append("printf 'Started app as %s\\n' \"$APP_USER\"")
    return "\n".join(command_lines) + "\n", primary_port
...
def _start_custom_command_for_deployment(deployment: RemoteDeployment, custom_command: str) -> str:
    ...
    command_lines = [
        ...
        "  OLD_PID=$(printf '%s\\n' \"$SUDO_PASS\" | sudo -S -u \"$APP_USER\" python3 -c 'import json,sys; print(json.load(open(sys.argv[1])).get(\"pid\", \"\"))' \"$RUNTIME_FILE\" 2>/dev/null || true)",
        "  if [ -n \"$OLD_PID\" ] && kill -0 \"$OLD_PID\" >/dev/null 2>&1; then",
        "    printf 'App already running with pid %s\\n' \"$OLD_PID\"",
        "    exit 0",
        "  fi",
        "fi",
        "printf '%s\\n' \"$SUDO_PASS\" | sudo -S -u \"$APP_USER\" sh -lc "
...
def list_startup_points(
    workspace_dir: Path,
    server_number: int,
    deployment_number: int,
) -> tuple[int, object | None, RemoteDeployment | None, list[dict] | None]:
    ...
    if code != 0 or device is None or deployment is None or username is None or password is None:
        return code, device, deployment, None
    startup_code, startup_points = _startup_points_for_deployment(device, username, password, deployment)
    return startup_code, device, deployment, startup_points
...
def start_remote_app(workspace_dir: Path, server_number: int, deployment_number: int) -> int:
    ...
    if fake_result != -1:
        return int(fake_result)
    startup_code, startup_points = _startup_points_for_deployment(device, username, password, deployment)
    if startup_code != 0 or startup_points is None:
        return startup_code
    try:
        command, _port = _start_command_for_deployment(deployment, startup_points)
    except RuntimeError as exc:
        print(str(exc))
        return 1
...
def start_remote_app_custom(
    ...
    if code != 0 or device is None or deployment is None or username is None or password is None:
        return code
    print(f"Starting custom command for {deployment.package_name} on {device.host} ({device.ip})...", flush=True)
    fake_result = _run_fake_deploy("start-custom", device, username, password, [deployment.package_name, custom_command])
    if fake_result != -1:
        return int(fake_result)
    completed = run_ssh_with_password(
        device,
...
def _stop_command_for_deployment(deployment: RemoteDeployment) -> str:
    ...
    return (
        ...
        "  exit 0\n"
        "fi\n"
        "PIDS=$(printf '%s\\n' \"$SUDO_PASS\" | sudo -S -u \"$APP_USER\" python3 - <<'PY' \"$RUNTIME_FILE\"\n"
        "import json, sys\n"
...
def format_startup_points(server_label: str, deployment: RemoteDeployment, startup_points: list[dict]) -> str:
    if not startup_points:
        return f"No startup points were detected for {deployment.package_name} on {server_label}."
    lines = [f"Startup points for {deployment.package_name} on {server_label}:"]
    for index, point in enumerate(startup_points, start=1):
        role = str(point.get("role", "companion"))
        name = str(point.get("name", f"process-{index}"))
        command = str(point.get("command", ""))
        source = str(point.get("source", "")).strip()
        lines.append(f"{index}. {role} {name}: {command}")
        if source:
